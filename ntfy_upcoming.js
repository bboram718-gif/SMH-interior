// SMH 1시간 전 알림 — GitHub Actions에서 5분마다 실행
...
async function main() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var category = String(r["구분"] || "").trim();
    var memoText = String(r["메모"] || "").trim();
    ...
    if (detailLine) {
      bodyLines.push(detailLine);
    }

    if (memoText) {
      bodyLines.push("  ✓ " + memoText);
    }

    if (!bodyLines.length) {
      bodyLines.push("일정 확인");
    }

    if (SMH_APP_URL) {
      bodyLines.push("");
      bodyLines.push("열기: " + SMH_APP_URL);
    }
